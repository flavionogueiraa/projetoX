const GLOBAL = {
  ...
  tableToExcel: (id_table, name_file) => {
    const date = new Date();
    const today = `${date.getFullYear()}_${
      date.getMonth() + 1
    }_${date.getDate()}`;
  ...
  getStringFilterFromForm(form) {
    ...
    const fieldsValues = fields.map(({ name, value }) => ({
      name: name,
      value: value,
    }));
  ...
  traversingMaquinario({ section, objForm, templateHTML }) {
    ...
    const arrayForm = Object.entries(objForm) // [ ['tipo'], [{}] ]
      .map(([key, value]) => {
        let objeto = {};
        objeto[key] = {
    ...
    Object.entries(objFields).map(([_, value]) => {
    ...
    inputs.forEach((inpt) =>
      inpt.classList.add("input-text", "h-48", "w-full")
    );
    labels.forEach((label) =>
      label.classList.add("label", "block", "text-neutral-50")
    );
  },
  ...
  formatReal: (v, style = "currency") =>
    new Intl.NumberFormat("pt-BR", { style: style, currency: "BRL" }).format(v),

  FormatREAL: (v) => {
    if (!v) return;
    const value = v.split(".");
    const [_, cents] = value;
    const money = Number(v).toLocaleString("pt-br", {
  ...
  defaultChartOptions: {
    ...
    colors: ["#BF1304", "#D93D04", "#F27405", "#F2A007", "#9BB8F2"],
  },
};
